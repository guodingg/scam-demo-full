...
// ─── Scene Demo ────────────────────────────────────────────────
function SceneDemo({ sceneId, scenes, onExit }) {
  ...
  // Fetch full scene data (includes stages) on mount or sceneId change
  useEffect(() => {
    if (!sceneId) return
    fetch(`${API}/scenes/${sceneId}`)
      .then(r => r.json())
      .then(d => { if (d.data) setSceneData(d.data) })
      .catch(() => {})
  }, [sceneId])
  ...
  useEffect(() => {
    ...
    fetch(`${API}/demonstrations`, {
      ...
      setDemoId(d.data.demonstration_id)
    })
  }, [sceneId])
  ...
  useEffect(() => {
    ...
    msgs.forEach((m, i) => {
      setTimeout(() => {
        setMessages(prev => [...prev, { who: m.who, text: m.text, time: m.time || '' }])
      }, delay + i * 800)
    })
  ...
  const handleChoice = (choice) => {
    ...
    // Record action
    if (demoId) {
      fetch(`${API}/demonstrations/${demoId}/actions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          stage_id: currentStage.id,
          action_type: 'choice',
          action_detail: choice.text,
          score_change: Math.max(0, choice.score),
          tags: choice.tags
        })
      })
    }
  ...
  const handleEndDemo = (finalScore, riskLevel, weakDims) => {
    if (!demoId) return
    fetch(`${API}/demonstrations/${demoId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        status: 'completed',
  ...
  const handleExit = () => {
    if (demoId) {
      handleEndDemo(score, score < 50 ? 'high' : score < 80 ? 'medium' : 'low', [])
    }
    onExit()
  }
  ...
  if (finished) {
    ...
    // Call immediately when finished state is entered (not in useEffect to avoid hook-order issues)
    if (demoId) {
      handleEndDemo(score, riskLevel, summary?.keyLessons?.map(l => '待加强项') || [])
    }
    ...
    return (
      <div className="container">
        <div className="edu-banner">
          <h2>🎓 演示完成</h2>
          <p>{activeScene.name} · {activeScene.estimated_time}分钟场景体验</p>
        </div>
        ...
        {summary?.keyLessons?.length > 0 && (
          <div className="card">
            ...
            <div className="key-lessons">
              <ul>
                {summary.keyLessons.map((l, i) => <li key={i}>{l}</li>)}
              </ul>
            </div>
          </div>
        )}
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        {currentStage?.riskPoints?.length > 0 && (
          <div className={`risk-panel ${showRisk ? 'show' : ''}`} style={{ margin: '0 16px 16px' }}>
            <h4>🚩 风险信号识别</h4>
            {currentStage.riskPoints.map((r, i) => (
              <div key={i} className="risk-item">
                <span className="risk-kw">"{r.keyword}"</span>
                <span className="risk-exp">{r.explanation}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="choices">
            {(currentStage?.choices || []).map((c, i) => {
              ...
              return (
                <button
                  key={i}
                  className={`choice-btn ${selectedChoice ? (selectedChoice.id === c.id ? c.type : 'disabled') : ''}`}
                  onClick={() => handleChoice(c)}
                  disabled={!!selectedChoice}
                >
                  <span className="ch-tag">{letters[i]}</span>
                  <span>{c.text}</span>
                </button>
              )
            })}
    ...
    <section id="stats" className="section active">
      <div className="card">
        ...
        <div className="stats-grid">
          <div className="stat-card">
            <div className="num">{data.total_demonstrations}</div>
            <div className="label">总演练人次</div>
          </div>
          <div className="stat-card">
            <div className="num">{data.avg_score}</div>
            <div className="label">平均得分</div>
          </div>
          <div className="stat-card">
            <div className="num">{data.high_risk_rate}%</div>
            <div className="label">高危人群占比</div>
          </div>
          <div className="stat-card">
            <div className="num">{data.high_risk_count}</div>
            <div className="label">高危人数</div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="bar-chart">
          {[
            { label: '0-20', key: '0-20', color: '#c0392b' },
            { label: '21-40', key: '21-40', color: '#e74c3c' },
            { label: '41-60', key: '41-60', color: '#f39c12' },
            { label: '61-80', key: '61-80', color: '#27ae60' },
            { label: '81-100', key: '81-100', color: '#27ae60' },
          ].map(r => {
        ...
        <div className="card">
          ...
          <div className="bar-chart">
            {data.scene_stats.map((s, i) => {
              const passRate = s.avg_score >= 60 ? Math.round((s.avg_score / 100) * 100) : Math.round((s.avg_score / 100) * 100)
              return (
                <div key={i} className="bar-row">
                  <span className="bar-label">{s.scene_id}</span>
                  <div className="bar-track"><div className="bar-fill" style={{ width: `${passRate}%` }} /></div>
    ...
    <section id="tips" className="section active">
      ...
      <div className="card">
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          {[
            {num:'1',t:'撒网',d:'通过短信、社交媒体、群聊发布"日赚300"等高薪兼职广告',color:'var(--danger)'},
            {num:'2',t:'诱饵',d:'前1-3单快速返佣金，建立信任让你觉得"这个兼职真的能赚钱"',color:'var(--warning)'},
            {num:'3',t:'放量',d:'逐步加大垫付金额，从500→2000→8000，利用"沉没成本"越陷越深',color:'#f59e0b'},
            {num:'4',t:'收割',d:'以"风控"、"解冻金"等借口继续施压，或直接消失',color:'#7c3aed'},
          ].map(x=>(
      ...
      <div className="card">
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {[
            {flag:'垫付资金',desc:'任何要求你先垫钱后返佣金，100%是诈骗',level:'danger'},
            {flag:'连单套牢',desc:'"任务必须连做N单才能提现"是最典型的刷单诈骗特征',level:'danger'},
            {flag:'大额垫付',desc:'从500元迅速攀升到数千元，目的是榨干你的全部存款',level:'danger'},
            {flag:'充值会员',desc:'以"保证金"、"解冻金"等名义要求充值，交钱即被套牢',level:'warning'},
            {flag:'高额佣金',desc:'30%-50%佣金远超正常电商刷单（3%-8%），必有诈',level:'warning'},
            {flag:'紧迫催促',desc:'以"名额有限"、"系统检测到风险"制造焦虑，阻止冷静思考',level:'warn'},
            {flag:'私人转账',desc:'要求向个人账户转账，正规平台使用对公账户',level:'danger'},
            {flag:'陌生链接',desc:'通过陌生链接或二维码下载App，可能含木马病毒',level:'danger'},
          ].map(x=>(
            <div key={x.flag} style={{padding:'9px 12px',borderRadius:8,border:'1px solid var(--border)',display:'flex',gap:8,alignItems:'flex-start'}}>
      ...
      {/* 冒充熟人诈骗 */}
      <div className="card">
        <h2 className="card-title"><span className="icon">👥</span> 冒充熟人诈骗</h2>
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {[
            {t:'盗号冒充',d:'诈骗分子盗取熟人的微信/QQ账号，利用信任直接借钱',icon:'🔓'},
            {t:'紧急求助',d:'以"出事"、"住院"、"被抓"等紧急情况施压，要求立刻转账',icon:'🆘'},
            {t:'拒绝验证',d:'找各种借口拒绝视频/语音通话，因为对方根本不是本人',icon:'📵'},
            {t:'逐级加码',d:'第一笔500，第二笔4500，第三笔8000...胃口只会越来越大',icon:'📈'},
          ].map(x=>(
            <div key={x.t} style={{padding:'8px 0',borderBottom:'1px solid var(--border)'}}>
              <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:3}}><span>{x.icon}</span><span style={{fontWeight:600,fontSize:'0.88rem'}}>{x.t}</span></div>
              <p style={{margin:0,fontSize:'0.83rem',color:'var(--text-light)',paddingLeft:22}}>{x.d}</p>
            </div>
          ))}
          <div style={{background:'#eff6ff',border:'1px solid #3b82f6',borderRadius:8,padding:'9px 12px',marginTop:2}}>
            <p style={{margin:0,fontSize:'0.85rem',color:'#1d4ed8'}}><span style={{fontWeight:700}}>✅ 防范：</span>任何熟人突然借钱，必须通过原有联系方式回拨确认，不接受任何"新号"替代</p>
          </div>
        </div>
      </div>
      ...
      {/* 虚假招聘诈骗 */}
      <div className="card">
        <h2 className="card-title"><span className="icon">💼</span> 虚假招聘诈骗</h2>
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {[
            {t:'高薪诱惑',d:'"日赚500+"、"在家办公"，利用贪心筛选受害者',icon:'💰'},
            {t:'绕过平台',d:'要求加个人微信或QQ联系，规避正规平台监管',icon:'📱'},
            {t:'收集信息',d:'以"注册账号"、"投递简历"为由收集手机号、身份证',icon:'📋'},
            {t:'充值会员',d:'成为会员才能接任务，99元-999元不等，交钱后越来越坑',icon:'💳'},
            {t:'任务套牢',d:'做任务要先垫付，垫付金额越来越大，从几十元到数千元',icon:'📈'},
          ].map(x=>(
            <div key={x.t} style={{display:'flex',gap:10,alignItems:'flex-start',padding:'7px 0',borderBottom:'1px solid var(--border)'}}>
              ...
              <div><span style={{fontWeight:600,fontSize:'0.86rem'}}>{x.t}</span><p style={{margin:2,fontSize:'0.81rem',color:'var(--text-light)'}}>{x.d}</p></div>
            </div>
          ))}
          <div style={{background:'#f0fdf4',border:'1px solid var(--safe)',borderRadius:8,padding:'9px 12px',marginTop:2}}>
            <p style={{margin:0,fontSize:'0.85rem',color:'var(--safe)'}}><span style={{fontWeight:700}}>✅ 防范：</span>正规求职渠道不会收取任何费用，所有"先交钱后干活"都是诈骗</p>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10}}>
          {[
            {t:'立刻停止',d:'发现需要垫付资金时，立刻停止是最优策略，不要试图"再做一单扳本"',icon:'🛑',color:'var(--danger)'},
            {t:'不要转账',d:'任何情况下都不要向个人账户转账，正规平台使用对公账户',icon:'🚫',color:'var(--danger)'},
            {t:'保留证据',d:'保留完整聊天记录截图、转账凭证、对方账号信息，是报警的关键',icon:'📸',color:'var(--warning)'},
            {t:'立即报警',d:'拨打110或96110（全国反诈热线），提供完整证据',icon:'📞',color:'var(--safe)'},
            {t:'不要沉默',d:'被骗后不要觉得丢人不敢报警，沉默只会让骗子继续害人',icon:'🔇',color:'var(--warning)'},
          ].map(x=>(
            <div key={x.t} style={{display:'flex',gap:10,alignItems:'flex-start',padding:'7px 0',borderBottom:'1px solid var(--border)'}}>
      ...
      <div className="card">
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12,padding:'4px 0'}}>
          {[
            {title:'一不垫',desc:'任何要求先垫付资金的"兼职"，都是诈骗。正规工作是先干活后收款，不是先付款后干活',color:'#dc2626'},
            {title:'二不扫',desc:'陌生二维码不扫描，可能是盗刷陷阱。正规付款通过官方App或官网进行',color:'#d97706'},
            {title:'三不信',desc:'"高薪轻松"、"稳赚不赔"、"限时优惠"都是话术，不要轻信任何夸大收益承诺',color:'#7c3aed'},
          ].map(x=>(
            <div key={x.title} style={{background:'var(--card-alt)',border:'1px solid var(--border)',borderRadius:10,padding:'12px 14px',display:'flex',gap:10,alignItems:'center'}}>
              <div style={{width:40,height:40,borderRadius:'50%',background:x.color,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontWeight:800,fontSize:'1rem',flexShrink:0}}>{x.title[0]}</div>
              <div><div style={{fontWeight:700,fontSize:'0.95rem',color:x.color,marginBottom:2}}>{x.title}</div><div style={{fontSize:'0.82rem',color:'var(--text-light)',lineHeight:1.4}}>{x.desc}</div></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card" style={{background:'linear-gradient(135deg,#fef2f2,#fee2e2)',border:'1px solid #fca5a5'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          {[
            {label:'全国反诈热线',value:'96110',note:'可打直接报警'},
            {label:'报警电话',value:'110',note:'24小时可拨打'},
            {label:'网络违法犯罪举报',value:'cyberpolice.online',note:'官网可在线举报'},
          ].map(x=>(
...
function RadarChart({ dimensions }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', margin: '0 auto' }}>
      {/* Grid circles */}
      {gridLevels.map(l => (
        <polygon key={l} points={polyGrid(l)} fill="none" stroke="#dcdde1" strokeWidth="0.5" opacity="0.6" />
      ))}
      {/* Axis lines */}
      {axisPoints.map((p, i) => (
        <line key={i} x1={center} y1={center} x2={p.x} y2={p.y} stroke="#dcdde1" strokeWidth="0.5" />
      ))}
      {/* Fill area */}
      ...
      {labels.map((label, i) => {
        ...
        return (
          <g key={label}>
            <text x={lx} y={ly} textAnchor="middle" dominantBaseline="central" fontSize="10" fill="var(--text)" fontWeight="600">
              {label}
            </text>
            <text x={lx} y={ly + 12} textAnchor="middle" dominantBaseline="central" fontSize="9" fill={val >= 60 ? 'var(--safe)' : 'var(--danger)'}>
              {val}
            </text>
          </g>
        )
...
// ─── Profile (Personal Center with Radar) ────────────────────
function Profile({ userId, scenes }) {
  const [data, setData] = useState(null)
  const [loading, setLoading] = useState(true)
  ...
  useEffect(() => {
    if (userId) { loadStats(); loadProfile(); }
  }, [userId]);
  ...
  const hasProfile = editForm.nickname || editForm.age_group;

  return (
    <section id="profile" className="section active">
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 className="card-title" style={{ margin: 0 }}><span className="icon">👤</span> 个人中心</h2>
        ...
        {!hasProfile && !showEdit && (
          <div style={{ marginTop: 12, background: 'linear-gradient(135deg,#fff8e1,#fff3cd)', border: '1px solid #ffc107', borderRadius: 10, padding: '12px 14px', fontSize: '0.85rem', color: '#856404' }}>
            👋 完成演练后别忘了完善个人信息，个性化分析您的防骗能力！
            <button onClick={() => setShowEdit(true)} style={{ marginLeft: 8, background: '#ffc107', color: '#856404', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: '0.8rem', cursor: 'pointer', fontWeight: 700 }}>去填写</button>
          </div>
        )}
      </div>
      ...
      <div className="card">
        <div className="stats-grid" style={{ marginBottom: 20 }}>
          <div className="stat-card">
            <div className="num">{totalDemos}</div>
            <div className="label">演练总次数</div>
          </div>
          <div className="stat-card">
          ...
          <div className="stat-card">
            <div className="num">{Object.keys(riskDist).length > 0 ? riskLabels[Object.keys(riskDist).reduce((a, b) => riskDist[a] > riskDist[b] ? a : b)] : '—'}</div>
        ...
        {totalDemos > 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: '30px 0', color: 'var(--text-light)', fontSize: '0.88rem' }}>
            完成演练后即可查看您的防骗能力雷达图
          </div>
        )}
      </div>
            ...
            {scenes.filter(s => sceneDist[s.id]).map(s => (
              <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border)', fontSize: '0.88rem' }}>
                <div>
                  ...
                  <span style={{ marginLeft: 8, color: 'var(--text-light)' }}>{s.name}</span>
                </div>
      ...
      {/* Risk distribution */}
      <div className="card">
        <h2 className="card-title"><span className="icon">📈</span> 风险分布</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {Object.entries(riskDist).length > 0 ? Object.entries(riskDist).map(([level, count]) => (
            <div key={level} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.88rem' }}>
              <span style={{ color: riskColors[level] || 'var(--text)', fontSize: '1.1rem' }}>{riskLabels[level]}</span>
              <div style={{ flex: 1, height: 8, background: 'var(--border)', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ width: `${(count / totalRisk) * 100}%`, height: '100%', background: riskColors[level] || 'var(--text)', borderRadius: 4 }} />
              </div>
              <span style={{ color: 'var(--text-light)', minWidth: 50, textAlign: 'right' }}>{count}次 ({Math.round(count / totalRisk * 100)}%)</span>
            </div>
          )) : (
            <div style={{ textAlign: 'center', padding: '20px 0', color: 'var(--text-light)', fontSize: '0.88rem' }}>暂无风险分布数据</div>
          )}
        </div>
      </div>
    </section>
...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${API}/scenes`).then(r => r.json()).then(d => setScenes(d.data || []))
  }, [])
  ...
  return (
    <div>
      ...
      <Nav active={nav} onNav={handleNav} />

      <div className="container">
        {nav === 'home' && <Home scenes={scenes} onStart={handleStart} />}
        {nav === 'tips' && <Tips />}
        {nav === 'stats' && <Stats />}
        {nav === 'profile' && <Profile userId={userId} scenes={scenes} />}
